...
def test_product_create(client: TestClient, category_factory, supplier_factory, admin_auth_header):
    ...
    response = client.post('/products/', headers=admin_auth_header,
                           json={
...
def test_product_update(client: TestClient, category_factory, supplier_factory, admin_auth_header):
    ...
    assert response.json()['price'] == 130
    assert response.json()['technical_details'] == 'bla bla2'